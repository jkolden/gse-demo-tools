...
procedure log_http_request (
    ...
    is
    /*
    This procedure is used to log the details of an HTTP request when logging is turned ON and the status code <> 200.
    The send and receive SOAP payloads are converted to BLOBs so the GSE support rep can download them from the APEX debug page.
    */
...
procedure log_http_request (
    ...
    /*
    This procedure is used to log the details of an HTTP request when logging is turned ON and the status code = 200.
    The request was successful so we don't need to examine the send and receive payloads.
    */
...
is
/*
This procedure was taken from Marc Sewtz's blog post: http://marcsewtz.blogspot.com/2008/04/generating-csv-files-and-storing-them.html
We use this code to build the content portion of the payloads for all SOAP API calls of importBulkData. 
*/
    l_cursor integer;
...
function parse_bulkload_response (p_clob IN clob)
   ...
   is
   /*
   This procedure parses the importBulkData SOAP response. The response contains the ESS job ID of the import process that is launched by the API. 
   There appears to be a bug in the API that sends extraneous characters before and after the response envelope that causes the 
   response payload to not be valid XML. Those must be stripped before we can parse the response
   */
    l_start number;
    l_end number;
    l_clob clob;
...
function build_ess_envelope(p_ess_id IN number)
    ...
    is
    /*
    This procedure build the SOAP API payload for the API that returns the status of a given ESS job ID
    */
...
function build_ess_envelope(p_job_package IN varchar2, p_job_definition IN varchar2)
    ...
    /*
    This procedure build the SOAP API payload for submitting an ERP ESS job.
    */
    l_envelope    clob;
...
IS
   --This procedure is from Steven Feuerstein's blog post on error handling: https://blogs.oracle.com/oraclemagazine/post/error-management
